...
static void setup_runtime_dir(void)
{
    ...
    if (old_runtime_dir) {
        ...
        MD_CHECK(g_old_runtime_dir != NULL);
    ...
    MD_CHECK(dir != NULL);
    MD_CHECK(strlen(dir) < sizeof(g_runtime_dir));
    strcpy(g_runtime_dir, dir);
    chmod(g_runtime_dir, 0700);

    MD_CHECK(setenv("XDG_RUNTIME_DIR", g_runtime_dir, 1) == 0);
...
static void test_invalid_names(void)
{
    ...
    MD_CHECK(md_ipc_listen(NULL) == NULL);
    MD_CHECK(md_ipc_connect(NULL, 1) == NULL);

    for (size_t i = 0; i < sizeof(invalid_names) / sizeof(invalid_names[0]); i++) {
        MD_CHECK(md_ipc_listen(invalid_names[i]) == NULL);
        MD_CHECK(md_ipc_connect(invalid_names[i], 1) == NULL);
    }
...
static void test_refuse_existing_non_socket(void)
{
    ...
    MD_CHECK(n > 0 && (size_t)n < sizeof(path));
...
static void test_connect_accept(void)
{
    ...
    MD_CHECK(args.accepted != NULL);

    /* Both ends should report connected */
    MD_CHECK(md_ipc_is_connected(client) == true);
    MD_CHECK(md_ipc_is_connected(args.accepted) == true);
...
static void test_send_recv(void)
{
    ...
    MD_CHECK(server_conn != NULL);

    /* Client → Server */
    const char *msg1 = "hello from client";
    MD_CHECK(md_ipc_send(client, msg1, strlen(msg1)) == 0);

    char buf[256];
    int n = md_ipc_recv(server_conn, buf, sizeof(buf), 2000);
    MD_CHECK(n == (int)strlen(msg1));
    MD_CHECK(memcmp(buf, msg1, (size_t)n) == 0);

    /* Server → Client */
    const char *msg2 = "hello from server";
    MD_CHECK(md_ipc_send(server_conn, msg2, strlen(msg2)) == 0);

    n = md_ipc_recv(client, buf, sizeof(buf), 2000);
    MD_CHECK(n == (int)strlen(msg2));
    MD_CHECK(memcmp(buf, msg2, (size_t)n) == 0);
...
static void test_send_large(void)
{
    ...
    MD_CHECK(md_ipc_send(client, data, sz) == 0);

    uint8_t *recv_buf = malloc(sz);
    MD_CHECK(recv_buf != NULL);
    int n = md_ipc_recv(server_conn, recv_buf, sz, 2000);
    MD_CHECK(n == (int)sz);
    MD_CHECK(memcmp(data, recv_buf, sz) == 0);
...
static void test_close_detection(void)
{
    ...
    MD_CHECK(n == 0); /* peer disconnected */
...
static void test_accept_timeout(void)
{
    ...
    MD_CHECK(conn == NULL); /* should timeout */
