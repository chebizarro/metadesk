...
static void test_request_roundtrip(void) {
    ...
    MD_CHECK(json != NULL);
    MD_CHECK(strlen(json) > 0);

    /* Verify expected fields are present */
    MD_CHECK(strstr(json, "\"session_request\"") != NULL);
    MD_CHECK(strstr(json, "\"v\":1") != NULL);
    MD_CHECK(strstr(json, "\"video\"") != NULL);
    MD_CHECK(strstr(json, "\"agent\"") != NULL);
    MD_CHECK(strstr(json, "\"input\"") != NULL);
    MD_CHECK(strstr(json, "\"compact\"") != NULL);
    MD_CHECK(strstr(json, "npub1testaddr") != NULL);

    /* Parse back and verify all fields */
    MdSessionRequest out;
    int ret = md_session_request_from_json(json, &out);
    MD_CHECK(ret == 0);
    MD_CHECK(out.capabilities == (MD_CAP_VIDEO | MD_CAP_AGENT | MD_CAP_INPUT));
    MD_CHECK(out.tree_format == MD_TREE_FORMAT_COMPACT);
    MD_CHECK(strcmp(out.fips_addr, "npub1testaddr") == 0);
...
static void test_accept_roundtrip(void) {
    ...
    MD_CHECK(strstr(json, "\"session_accept\"") != NULL);
    MD_CHECK(strstr(json, "\"v\":1") != NULL);
    MD_CHECK(strstr(json, "a1b2c3d4-e5f6-7890-abcd-ef1234567890") != NULL);
    MD_CHECK(strstr(json, "\"video\"") != NULL);
    MD_CHECK(strstr(json, "\"agent\"") != NULL);
    ...
    MD_CHECK(ret == 0);
    MD_CHECK(out.granted == (MD_CAP_VIDEO | MD_CAP_AGENT));
    MD_CHECK(strcmp(out.session_id, "a1b2c3d4-e5f6-7890-abcd-ef1234567890") == 0);
    MD_CHECK(out.tree_format == MD_TREE_FORMAT_COMPACT);
...
static void test_single_capability(void) {
    ...
    MdSessionRequest out;
    MD_CHECK(md_session_request_from_json(json, &out) == 0);
    MD_CHECK(out.capabilities == MD_CAP_VIDEO);
...
static void test_tree_format_json(void) {
    ...
    char *json = md_session_request_to_json(&req);
    MD_CHECK(json != NULL);
    MD_CHECK(strstr(json, "\"json\"") != NULL);

    MdSessionRequest out;
    MD_CHECK(md_session_request_from_json(json, &out) == 0);
    MD_CHECK(out.tree_format == MD_TREE_FORMAT_JSON);
...
static void test_empty_session_id(void) {
    ...
    MdSessionAccept out;
    MD_CHECK(md_session_accept_from_json(json, &out) == 0);
    MD_CHECK(out.session_id[0] == '\0');
    MD_CHECK(out.granted == MD_CAP_VIDEO);
...
static void test_wrong_type_rejected(void) {
    ...
    MD_CHECK(ret == -1); /* wrong type → parse failure */
...
static void test_null_args(void) {
    ...
    MD_CHECK(md_session_request_to_json(NULL) == NULL);
    MD_CHECK(md_session_accept_to_json(NULL) == NULL);

    /* NULL json → error */
    MdSessionRequest req;
    MD_CHECK(md_session_request_from_json(NULL, &req) == -1);
    MD_CHECK(md_session_request_from_json("{}", NULL) == -1);

    MdSessionAccept acc;
    MD_CHECK(md_session_accept_from_json(NULL, &acc) == -1);
    MD_CHECK(md_session_accept_from_json("{}", NULL) == -1);

    /* Invalid JSON → error */
    MD_CHECK(md_session_request_from_json("not json", &req) == -1);
    MD_CHECK(md_session_accept_from_json("{broken", &acc) == -1);

    /* Valid JSON but missing type → error */
    MD_CHECK(md_session_request_from_json("{\"v\":1}", &req) == -1);
    MD_CHECK(md_session_accept_from_json("{\"v\":1}", &acc) == -1);
...
static void test_caps_helpers(void) {
    ...
    MD_CHECK(found_video);
    MD_CHECK(found_input);
...
static void test_state_machine(void) {
    MdSession s;
    md_session_init(&s);
    MD_CHECK(s.state == MD_SESSION_IDLE);
    MD_CHECK(s.keepalive_ms == MD_SESSION_KEEPALIVE_DEFAULT_MS);

    /* Host side can accept an incoming session from IDLE */
    MD_CHECK(md_session_accept(&s, "host-side-id", MD_CAP_VIDEO) == 0);
    MD_CHECK(s.state == MD_SESSION_NEGOTIATING);
    MD_CHECK(strcmp(s.session_id, "host-side-id") == 0);
    ...
    MD_CHECK(md_session_activate(&s) == -1);

    /* Request transitions to REQUESTING */
    MD_CHECK(md_session_request(&s, "npub1abc", MD_CAP_VIDEO | MD_CAP_AGENT,
                              MD_TREE_FORMAT_COMPACT) == 0);
    MD_CHECK(s.state == MD_SESSION_REQUESTING);
    MD_CHECK(s.capabilities == (MD_CAP_VIDEO | MD_CAP_AGENT));
    MD_CHECK(s.tree_format == MD_TREE_FORMAT_COMPACT);
    MD_CHECK(strcmp(s.peer_npub, "npub1abc") == 0);

    /* Can't request again from REQUESTING */
    MD_CHECK(md_session_request(&s, "npub1xyz", MD_CAP_VIDEO, MD_TREE_FORMAT_JSON) == -1);

    /* Accept transitions to NEGOTIATING */
    MD_CHECK(md_session_accept(&s, "session-uuid-1", MD_CAP_VIDEO) == 0);
    MD_CHECK(s.state == MD_SESSION_NEGOTIATING);
    MD_CHECK(s.capabilities == MD_CAP_VIDEO); /* granted overrides requested */
    MD_CHECK(strcmp(s.session_id, "session-uuid-1") == 0);

    /* Activate transitions to ACTIVE */
    MD_CHECK(md_session_activate(&s) == 0);
    MD_CHECK(s.state == MD_SESSION_ACTIVE);

    /* Disconnect from ACTIVE */
    MD_CHECK(md_session_disconnect(&s) == 0);
    MD_CHECK(s.state == MD_SESSION_DISCONNECTING);

    /* Reset is the path from DISCONNECTING back to IDLE */
    md_session_reset(&s);
    MD_CHECK(s.state == MD_SESSION_IDLE);
    MD_CHECK(s.session_id[0] == '\0');

    /* Can't disconnect from IDLE */
    md_session_init(&s);
    MD_CHECK(md_session_disconnect(&s) == -1);
...
static void test_accept_from_idle_host_side(void) {
    ...
    MD_CHECK(md_session_accept(&s, "host-side-session", MD_CAP_VIDEO | MD_CAP_AGENT) == 0);
    MD_CHECK(s.state == MD_SESSION_NEGOTIATING);
    MD_CHECK(s.capabilities == (MD_CAP_VIDEO | MD_CAP_AGENT));
    MD_CHECK(strcmp(s.session_id, "host-side-session") == 0);
...
static void test_reset_after_disconnect(void) {
    ...
    MD_CHECK(md_session_request(&s, "npub1peer", MD_CAP_VIDEO,
                              MD_TREE_FORMAT_COMPACT) == 0);
    MD_CHECK(md_session_accept(&s, "disconnect-reset-session", MD_CAP_VIDEO) == 0);
    MD_CHECK(md_session_activate(&s) == 0);
    MD_CHECK(md_session_disconnect(&s) == 0);
    MD_CHECK(s.state == MD_SESSION_DISCONNECTING);
    ...
    md_session_reset(&s);
    MD_CHECK(s.state == MD_SESSION_IDLE);
    MD_CHECK(s.capabilities == 0);
    MD_CHECK(s.tree_format == MD_TREE_FORMAT_JSON);
    MD_CHECK(s.session_id[0] == '\0');
    MD_CHECK(s.peer_npub[0] == '\0');
    MD_CHECK(s.keepalive_ms == MD_SESSION_KEEPALIVE_DEFAULT_MS);
...
static void test_invalid_capability_strings_rejected(void) {
    ...
    const char *bad_req = "{\"type\":\"session_request\",\"v\":1,"
                          ...
                          "\"tree_format\":\"json\"}";
    MD_CHECK(md_session_request_from_json(bad_req, &req) == -1);

    const char *bad_req_type = "{\"type\":\"session_request\",\"v\":1,"
                               "\"capabilities\":[\"video\",42],"
                               "\"tree_format\":\"json\"}";
    MD_CHECK(md_session_request_from_json(bad_req_type, &req) == -1);
    ...
    const char *bad_acc = "{\"type\":\"session_accept\",\"v\":1,"
                          ...
                          "\"tree_format\":\"compact\"}";
    MD_CHECK(md_session_accept_from_json(bad_acc, &acc) == -1);
...
static void test_keepalive_timeout(void) {
    ...
    MD_CHECK(!md_session_is_timed_out(&s, 100000));

    /* Record a pong */
    md_session_on_pong(&s, 1000);
    MD_CHECK(s.last_pong_ms == 1000);

    /* Not timed out shortly after pong */
    MD_CHECK(!md_session_is_timed_out(&s, 1000 + s.keepalive_ms));

    /* Clock rollback must not underflow into a false timeout */
    MD_CHECK(!md_session_is_timed_out(&s, 999));

    /* Timed out after keepalive * timeout_mult */
    uint64_t timeout = (uint64_t)s.keepalive_ms * MD_SESSION_KEEPALIVE_TIMEOUT_MULT;
    MD_CHECK(!md_session_is_timed_out(&s, 1000 + timeout));
    MD_CHECK(md_session_is_timed_out(&s, 1000 + timeout + 1));

    /* Ping returns true when ACTIVE */
    MD_CHECK(md_session_on_ping(&s, 2000) == true);
    MD_CHECK(s.last_ping_ms == 2000);
...
static void test_null_session(void) {
    ...
    MD_CHECK(md_session_request(NULL, "npub", 0, MD_TREE_FORMAT_JSON) == -1);
    MD_CHECK(md_session_accept(NULL, "id", 0) == -1);
    MD_CHECK(md_session_activate(NULL) == -1);
    MD_CHECK(md_session_disconnect(NULL) == -1);
    md_session_reset(NULL);
    MD_CHECK(md_session_on_ping(NULL, 0) == false);
    md_session_on_pong(NULL, 0);
    MD_CHECK(md_session_is_timed_out(NULL, 0) == false);
...
static void test_accept_tree_format_negotiation(void) {
    ...
    MD_CHECK(md_session_accept_from_json(no_tf, &out) == 0);
    MD_CHECK(out.tree_format == MD_TREE_FORMAT_JSON);
    ...
    MD_CHECK(req_json != NULL);

    MdSessionRequest parsed_req;
    MD_CHECK(md_session_request_from_json(req_json, &parsed_req) == 0);
    MD_CHECK(parsed_req.tree_format == MD_TREE_FORMAT_COMPACT);
    free(req_json);
    ...
    MD_CHECK(acc_json != NULL);

    MdSessionAccept client_acc;
    MD_CHECK(md_session_accept_from_json(acc_json, &client_acc) == 0);
    MD_CHECK(client_acc.tree_format == MD_TREE_FORMAT_COMPACT);
    free(acc_json);
